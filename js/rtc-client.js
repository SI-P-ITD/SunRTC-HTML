  ...
  async join() {
    ...
    //加入房间成功回调回调
	  var roomMembers = states.length+1;
		...
		getEnterRoomSuccess(sessionStorage.getItem("userId"),sessionStorage.getItem("roomId"),roomMembers++,timestamp);
...
//						data.resultContent.theFirstOneEnter //Y为第一个进入房间 N不是第一个
			...
			var ajax = new MYAJAX("enterRoomSuccess", "h5", para, onSuccess);
		}
